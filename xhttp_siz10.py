# xhttp_siz10.py — نسخه نهایی با مدیریت خطاهای ClientDisconnect
# تمامی خطاهای احتمالی در حین خواندن body از request یا stream
# به‌طور کامل پوشش داده شده‌اند تا از کرش‌های 500 جلوگیری شود.
...
from starlette.requests import ClientDisconnect  # اضافه کردن import
...
async def packet_up_upload(uuid: str, session_id: str, seq: int, request: Request):
    ...
    # 🔧 مهم: گرفتن خطای ClientDisconnect هنگام خواندن body
    try:
        body = await request.body()
    except ClientDisconnect:
        # کلاینت قطع کرده، جلسه رو تمیز می‌کنیم و خطای 400 برمی‌گردونیم
        await _teardown(session_id)
...
async def stream_up_upload(uuid: str, session_id: str, request: Request):
    ...
    try:
        ...
        # کلاینت ارتباط رو ناگهانی قطع کرد، جلسه رو تمیز می‌کنیم
        await gate.flush()
        await _teardown(session_id)
        # می‌تونیم خطای 400 برگردونیم ولی دیگه کلاینت گوش نمی‌ده،
        # فقط جلسه بسته شده و خطای 502 که قبلاً رخ می‌داد دیگه اتفاق نمی‌افته
        raise HTTPException(status_code=400, detail="client disconnected")
